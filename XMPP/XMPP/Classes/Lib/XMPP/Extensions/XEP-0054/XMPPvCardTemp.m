...
- (NSArray *)emailAddresses {
    ...
    NSXMLElement *emailEl = [self elementForName:@"EMAIL"];

    NSXMLElement * userIdEl = [emailEl elementForName:@"USERID"];
    
    NSString *email = [userIdEl stringValue];
    if (email.length > 0) {
...
- (void)setEmailAddresses:(NSArray *)emails {
    ...
    NSXMLElement * userIdEl = [emailEl elementForName:@"USERID"];
    NSString *email = [userIdEl stringValue];
    [userIdEl removeAttributeForName:email];


        
    NSString *emlStr = emails[0];
    NSXMLElement *userID = [NSXMLElement elementWithName:[userIdEl stringValue] stringValue:emlStr];
    [userIdEl addChild:userID];
}
